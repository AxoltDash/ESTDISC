    ...
    \item \marginnote{\it4 puntos} Demuestre utilizando inducci\'on que las siguientes f\'ormulas se cumplen.			
        ...
        \begin{itemize}
          ...
          \item Hip\'otesis inductiva\\ (Suponer que la propiedad se cumple para "n")
            ...
            % \item Supongamos que m = n-1 y que: $\sum\limits_{k=1}^m k\,(k!) = (n)!-1$
            % Por paso inductivo es demostrar ($m = n-1$) y que: $\sum\limits_{k=1}^m k\,(k!) = (n)!-1$\\\\
            % Supongamos que ($m = n-1$) y que: $\sum\limits_{k=1}^m k\,(k!) = (n)!-1$\\\\
            % Paso inductivo: Por demostrar: $\sum\limits_{k=1}^n k\,(k!) = (n+1)!-1$\\\\
            % PIPIPIIPIPIPI
